...
let
  ...
  dotfiles = import <hs/dotfiles/default.nix>
  ...
  context.dotFilePaths = [
    <hs/dotfiles/programs.nix>
    <hs/dotfiles/nvim.nix>
    <hs/dotfiles/xfce4-terminal.nix>
    <hs/dotfiles/gitconfig.nix>
    <hs/dotfiles/gitignore.nix>
    <hs/dotfiles/nix.nix>
    <hs/dotfiles/oath.nix>
    <hs/dotfiles/jstools.nix>
    <hs/dotfiles/superslicer.nix>
    <hs/dotfiles/scan.nix>
  ];
  ...
  context.variables = rec {
    ...
    programs = {
      ...
      dropdown = "${dotFileAt <hs/dotfiles/i3config.nix> 1} --role=ScratchTerm";
      browser = "${profileDir}/bin/firefox";
      editor = "${nano}/bin/nano";
      launcher = dotFileAt <hs/dotfiles/bemenu.nix> 0;
      window-size = dotFileAt <hs/dotfiles/i3config.nix> 2;
      window-center = dotFileAt <hs/dotfiles/i3config.nix> 3;
...
in {
    nixpkgs.config = import <hs/dotfiles/nixpkgs-config.nix>;
    xdg = {
      enable = true;
      configFile."nixpkgs/config.nix".source = <hs/dotfiles/nixpkgs-config.nix>;
    };
    ...
    home.packages = with pkgs; [
      ...
      socat
      protonvpn-cli
      (import "${nixmySrc}/default.nix" { inherit pkgs lib; config = config.nixpkgs.config; })
    ];
      ...
      config = rec {
        ...
        keybindings =
          mkOptionDefault {
            ...
            "${modifier}+Control+Left" = "exec WSNUM=$(${dotFileAt <hs/dotfiles/i3_workspace.nix> 0} --skip prev) && ${context.variables.i3-msg} workspace $WSNUM";
            "${modifier}+Control+Right" = "exec WSNUM=$(${dotFileAt <hs/dotfiles/i3_workspace.nix> 0} --skip next) && ${context.variables.i3-msg} workspace $WSNUM";
            "${modifier}+Control+Shift+Left" = "exec WSNUM=$(${dotFileAt <hs/dotfiles/i3_workspace.nix> 0} prev) && ${context.variables.i3-msg} move workspace $WSNUM && ${context.variables.i3-msg} workspace $WSNUM";
            "${modifier}+Control+Shift+Right" = "exec WSNUM=$(${dotFileAt <hs/dotfiles/i3_workspace.nix> 0} next) && ${context.variables.i3-msg} move workspace $WSNUM && ${context.variables.i3-msg} workspace $WSNUM";
          };
          modifier = "Mod1";
          startup = [
  ...
  programs.zsh = {
    ...
    initExtra = ''
      ${readFile (dotFileAt <hs/dotfiles/zsh.nix> 0)}

      . "${pkgs.nix}/etc/profile.d/nix.sh"
      . "${config.home.profileDirectory}/etc/profile.d/hm-session-vars.sh"
    '';
    loginExtra = readFile (dotFileAt <hs/dotfiles/zsh.nix> 1);
  };
  programs.starship = {
    enable = true;
    settings = {
